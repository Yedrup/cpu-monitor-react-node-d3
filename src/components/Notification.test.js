...
const configExample = {
  loadAverageByCpuConsideredAsHigh: 0.6,
...
it('should create an alert of High Load Average confirmed', () => {
  ...
  let currentSuspectedWindowAverage = parseFloat(
    calculateTracesArrayAverage(tracesExamples)
  );
  let isWindowMinToConfirmHighAverageReached =
    tracesExamples.length >= configExample.getHighLoadAverageMinArrayLength();
  let isCurrentlyInHighAverage =
    currentSuspectedWindowAverage >
    configExample.loadAverageByCpuConsideredAsHigh;

  if (isCurrentlyInHighAverage) {
    if (isWindowMinToConfirmHighAverageReached) {
      expect(
        isCurrentlyInHighAverage && isWindowMinToConfirmHighAverageReached
      ).toBe(true);
    }
  } else {
