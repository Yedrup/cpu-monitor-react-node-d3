...
export const getTplTemporaryReport = (report) => {
  return `
        ...
        <p>🕙${LABELS.chart.tooltip.common.eventInfo.started}: ${
    report.startDateString
  } </p>
...
export const getTplFinalReport = (report) => {
  ...
  return `
        <h1>${LABELS.chart.tooltip[report.type].title}</h1>
        <p>🕙 ${
          LABELS.chart.tooltip.common.eventInfo.started
        }: ${startDateString} </p >
        <p>🕥 ${
          LABELS.chart.tooltip.common.eventInfo.finished
        }: ${endDateString} </p >
        <p>⏱ ${LABELS.chart.tooltip.common.duration}: ${duration}</p>
        <p>🏷 ${
          LABELS.chart.tooltip.common.average.onPeriod
        }: ${completeAverage}</p>
