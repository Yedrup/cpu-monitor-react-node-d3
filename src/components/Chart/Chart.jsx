...
import {
  getTplTracePoint,
  getTplTemporaryReport,
  getTplFinalReport,
} from './utilities/Tooltips';
...
const useStyles = makeStyles((theme) => {
  ...
  return {
    ...
    tooltipVisible: {
      ...
      font: '1rem',
      fontWeight: 'bold',
      backgroundColor: fade(theme.palette.primary.darker, 0.8),
    ...
    axisLabel: {
      ...
      stroke: fade(theme.palette.primary.contrastText, 0.2),
      fontSize: '1.25rem',
    },
    axisSubtitleLabel: {
      fill: isDarkTheme ? theme.palette.grey[400] : theme.palette.grey[600],
      stroke: isDarkTheme
        ? fade(theme.palette.grey[400], 0.2)
        : fade(theme.palette.grey[600], 0.2),
      fontSize: '1rem',
    },
    axisSubtitleLabelOverlined: {
      stroke: fade(theme.palette.primary.dark, 0.8),
    ...
    legend: {
      fill: fade(theme.palette.primary.main, 0.1),
    },
    legendLabel: {
      fill: isDarkTheme ? theme.palette.grey[400] : theme.palette.grey[600],
      fontWeight: 'bold',
    ...
    frameHighLoad: {
      fill: isDarkTheme
        ? fade(theme.palette.error.main, 0.4)
        : fade(theme.palette.error.main, 0.5),
    },
    frameRecovery: {
      fill: isDarkTheme
        ? fade(theme.palette.success.main, 0.4)
...
function Chart(props) {
  ...
  const { loadAverageByCpuConsideredAsHigh } = stateConfig;
  ...
  let formattedWhiteSpacedLabel = useMemo(
    () =>
      addPlaceholder(
        LABELS.chart.axis.xAxisLabelSubtitle,
        /PLACEHOLDER/gi,
  ...
  const renderChart = ({
    ...
    const svg = select(svgElem).attr('class', `${classes.svg}`);
    ...
    const xAxisG = mainGroup
      .append('g')
      .attr('class', 'x-axis-group')
      .attr('transform', `translate(0, ${innerHeight})`)
      .call(xAxis);

    const xAxisTextG = xAxisG
      .append('g')
    ...
    const subLabelComposed = xAxisTextG
      .append('text')
    ...
    const getStartDateForDisplayFrame = (report) => {
      ...
      return olderTimeTrace < report.startDateInMs
        ? report.startDateInMs
        : olderTimeTrace;
    };
    ...
    if (isHighLoadInProgress) {
      ...
      const framesTemporaryHighLoadInProgress = timeFrameGroup
        ...
        .on('mouseover', (d) => {
          ...
          tooltip
            .html(tooltipTpl(getTplTemporaryReport(d)))
    ...
    framesHighLoad
      ...
      .on('mouseover', (d) => {
        tooltip.classed(`${classes.tooltipVisible}`, true);
        tooltip
          .html(tooltipTpl(getTplFinalReport(d)))
          .style('left', `${event.pageX}px`)
          .style('top', `${event.pageY - 10}px`);
      })
      .on('mouseout', (d) => {
    ...
    maxLoadAverage
      ...
      .attr('x1', 0)
      .attr('y1', yScale(loadAverageByCpuConsideredAsHigh))
      .attr('x2', innerWidth)
      .attr('y2', yScale(loadAverageByCpuConsideredAsHigh));
    maxLoadAverage
      .append('text')
      .attr('y', yScale(loadAverageByCpuConsideredAsHigh));
    ...
    lineGroup
      ...
      .on('mouseover', (d) => {
        tooltip.classed(`${classes.tooltipVisible}`, true);
        tooltip
          .html(tooltipTpl(getTplTracePoint(d)))
          .style('left', `${event.pageX}px`)
    ...
    const legendGroupEnter = mainGroup
      .append('g')
      .attr(
        'transform',
        `translate(0, ${
          innerHeight - legendHeight + margin.bottom - legendYOffset
        })`
      );
    ...
    dashGroup
      ...
      .text(
        `${LABELS.chart.legend.limit.title} : ${loadAverageByCpuConsideredAsHigh}`
      )
      .attr('dy', 10)
  ...
  return (
    <Card display="flex" elevation={2}>
      <CardHeaderCustom title={title} />
      <CardContent className={classes.cardContent}>
        <svg viewBox={`0 0 ${850} ${650} `} ref={svgElementRef}></svg>
      </CardContent>
    </Card>
  );
}
