...
function ReportTable({ report, index }) {
  ...
  const useStyles = makeStyles((theme) => ({
    ...
    subCategory: {
      backgroundColor: fade(theme.palette.primary.light, 0.02),
      marginBottom: '2rem',
    },
    list: {
      display: 'flex',
  ...
  const StyledReportCard = withStyles((theme) => ({
    root: {
      ...
      '&:nth-of-type(even)': {
        backgroundColor: fade(theme.palette.tile.backgroundColor, 0.8),
      },
      padding: '1rem',
      marginBottom: '1rem',
    },
  ...
  return (
    <StyledReportCard m={6}>
        ...
        <CardContent>
          <Typography component="div" align="left">
            <List component="ul" direction="row" className={classes.list}>
              {overview.map(
                ({ Icon, label, value, color, classCus }, index) => {
                  return (
      ...
      <Box mt={5}>
        <Typography align="left" variant="h3" mt={4} color="primary">
          Traces
        </Typography>
              ...
              {traces.map((row) => {
                return (
                  <TableRowCustom>
                    ...
                    <TableCellCustom align="center">
                      {row.loadAverageLast1Min}
                    </TableCellCustom>
                    <TableCellCustom align="center">
                      {row.loadAverageLast5Mins}
                    </TableCellCustom>
                    <TableCellCustom align="center">
                      {row.loadAverageLast15Mins}
                    </TableCellCustom>
                  </TableRowCustom>
                );
              })}
