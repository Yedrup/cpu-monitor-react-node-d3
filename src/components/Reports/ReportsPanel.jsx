...
import { Paper, Tabs, Tab, Typography, Avatar, Box } from '@material-ui/core';
...
const useStyles = makeStyles((theme) => {
  return {
    ...
    event: {
      ...
      '&:nth-of-type(even)': {
        backgroundColor: fade(theme.palette.primary.darker, 0.1),
      },
    },
    eventTitle: {
      padding: '.5rem 1rem',
...
function WrappedReportsPanel({ reports }) {
  ...
  const { eventsFinalReports } = reports;

  const eventsFinalReportsCount = eventsFinalReports?.length || 0;

  let highLoadReports = filterArrayOfObjectByProperty(
    eventsFinalReports,
    'highLoadReports'
  );
  let recoveryReports = filterArrayOfObjectByProperty(
    eventsFinalReports,
    'recoveryReports'
  );
  // TABS
  ...
  const getTabContentTitle = (tabDataType, arrayLength) => {
    return !arrayLength
      ? LABELS.tableReport.common.noReport
      : arrayLength && arrayLength > 1
      ? LABELS.tableReport[tabDataType].reports
      : LABELS.tableReport[tabDataType].report;
  };
  return (
    <Paper elevation={2}>
      <Tabs
      ...
      <TabPanel value={currentTabVal} index={0}>
        <Typography className={classes.tabContentTitle} component="div">
          <Typography
            className={clsx(
              ...
              classes.tabContentTitleFlexStart
            )}
            variant="h2"
          >
            <Avatar
        ...
        {eventsFinalReports &&
          Object.values(eventsFinalReports).map((currentEvent, index) => {
            const { highLoadReports, recoveryReports } = currentEvent;
            return (
              <div key={index} className={classes.event}>
                <Typography className={classes.eventTitle} variant="h2">
                  Event n° {index + 1}
                </Typography>
      ...
      <TabPanel value={currentTabVal} index={1}>
        ...
        {highLoadReports &&
          highLoadReports.map((report, ind) => (
            <ReportTable key={ind} report={report} index={ind} />
          ))}
      </TabPanel>
      <TabPanel value={currentTabVal} index={2}>
        ...
        {recoveryReports &&
          recoveryReports.map((report, ind) => (
            <ReportTable key={ind} report={report} index={ind} />
          ))}
      </TabPanel>
    </Paper>
...
function compareReports(prevProps, nextProps) {
  ...
  if (nextProps.reports.eventsFinalReports.length) {
    ...
    let isSameCountOfReports =
      prevEventsReports.length === nextEventsReports.length;
    // we check if the oldest report is the same (in case of LRU) the count of reports is not enough to attest they are the same
    let isSameOldestReport =
      prevEventsOldestReport?.highLoadReports.startDateInMs ===
      nextEventsOldestReport.highLoadReports.startDateInMs;
