...
const updateReportsDisplayed = (
  highLoadFinalReportsToDisplay,
  recoveryFinalReportsToDisplay,
  valueToCompareTo
) => {
  // update the reports to be displayed in the time window of the chart
  let updatedHightLoadFinalReportsDisplayed = removeFromTracesArrToDisplayLRU(
    highLoadFinalReportsToDisplay,
    'endDateInMs',
    valueToCompareTo
  );
  let updatedRecoveryFinalReportsDisplayed = removeFromTracesArrToDisplayLRU(
    recoveryFinalReportsToDisplay,
    'endDateInMs',
    valueToCompareTo
  );
...
const updateEventsHistoric = (eventsFinalReports, lastTrace = {}) => {
  ...
  let currentHistoricTimeWindowInMs =
    lastTrace.configUsed?.timeHistoryWindowInMs;
  let updated = eventsHistoricLRU(
    eventsFinalReports,
    'endDateInMs',
    currentHistoricTimeWindowInMs
  );
  return updated;
};

const estimateLastXMinutesAverage = (traces, lastTrace) => {
  let timeWindowArrayLength =
    lastTrace?.configUsed?.getTimeWindowArrayLength() || 2;
  if (traces.length < timeWindowArrayLength) {
    let estimate =
      (lastTrace?.loadAverageLast5Mins + lastTrace?.loadAverageLast15Mins) / 2;
...
export const DataReducer = (state, action) => {
  let lastTrace = state.traces.length
  ...
  switch (action.type) {
    case 'UPDATE_TRACES':
      ...
      let {
        updatedHightLoadFinalReportsDisplayed,
        updatedRecoveryFinalReportsDisplayed,
      } = updateReportsDisplayed(
        state.highLoadFinalReportsToDisplay,
        state.recoveryFinalReportsToDisplay,
        olderTraceTimeInMs
      );
      ...
      let {
        ...
        lastWindowAverage,
      } = estimateLastXMinutesAverage(updatedTraces, updatedLastTrace);

      let updatedHistoric = updateEventsHistoric(
        state.eventsFinalReports,
        updatedLastTrace
      );
    ...
    case 'UPDATE_REPORT_HIGH_LOAD_IN_PROGRESS':
      ...
      let updatedTracesDisplay = removeFromTracesArrToDisplayLRU(
        tempReportTraces,
        'dateInMs',
        olderTraceTimeInMs
      );
      return {
        ...
        highLoadTempReportToDisplay: {
          ...state.highLoadTempReportToDisplay,
          traces: updatedTracesDisplay,
          lastUpdateInMs: lastTraceTimeInMs,
        },
      };
    case 'UPDATE_FINAL_REPORTS':
      return {
        ...
        highLoadFinalReportsToDisplay: [
          ...state.highLoadFinalReportsToDisplay,
          action.payload.highLoadReports,
        ],
        recoveryFinalReportsToDisplay: [
          ...state.recoveryFinalReportsToDisplay,
          action.payload.recoveryReports,
        ],
        highLoadTempReportToDisplay: null,
        isHighLoadCurrentlyInProgress: false,
      };
